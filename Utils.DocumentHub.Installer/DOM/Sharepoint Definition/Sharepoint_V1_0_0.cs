namespace Skyline.DataMiner.Utils.DocumentHub.Installer.DOM.Sharepoint_Definition
{
	using System;
	...
	internal class Sharepoint_V1_0_0 : DomMigration
	{
		...
		public override void Migrate()
		{
			var section = new SectionDefinitionBuilder()
				...
				.AddFieldDescriptor(new FieldDescriptorBuilder()
					.WithID(SharePointConfigurationDomMapper.SharePointConfigurationProperties.ClientSecret)
					.WithName(nameof(SharePointConfigurationDomMapper.SharePointConfigurationProperties.ClientSecret))
					.WithType(typeof(string))
					.WithIsOptional(true)
					.WithTooltip("The client secret for authentication with the SharePoint application"))
				.AddFieldDescriptor(new FieldDescriptorBuilder()
