...
namespace Skyline.DataMiner.Utils.DocumentHub.SDM
{
	...
	{
		internal static FilterElement<T> Create<T, TValue>(Exposer<T, TValue> exposer, Comparer comparer, TValue value)
		{
			if (exposer is null)
			{
		...
		internal static FilterElement<T> Create<T, TValue>(DynamicListExposer<T, TValue> exposer, Comparer comparer, TValue value)
		{
			if (exposer is null)
			{
				throw new ArgumentNullException(nameof(exposer));
